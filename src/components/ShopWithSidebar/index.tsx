...
import { categoriesAtom, newArrivalsAtom } from "@/store";
...
const ShopWithSidebar = () => {
  ...
  const [products] = useAtom(newArrivalsAtom);
  ...
  const fetchAndSetProducts = async (page: number, categoryId: string) => {
    try {
      const data = await fetchProducts({ parent_category: categoryId, page });
  ...
  useEffect(() => {
    fetchAndSetProducts(page, category);
  }, [page, category]);
  console.log(count);
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between" onClick={clearCategoryFilter}>
                      <p>Фильтры:</p>
                      <button className="text-blue">Очистить все</button>
                    </div>
                  </div>
          ...
          <div className="flex justify-center mt-6">
            ...
            <span className="px-4 py-2">
              Страница {page} из {Math.ceil(count / ITEMS_PER_PAGE)}
            </span>
            <button
              onClick={() => handlePageChange(page + 1)}
